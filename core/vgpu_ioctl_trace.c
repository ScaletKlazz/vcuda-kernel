...
struct vgpu_rm_control_trace_entry {
	bool used;
	struct vgpu_rm_control_trace_snapshot snapshot;
};
...
static struct vgpu_rm_control_trace_entry
	vgpu_rm_control_trace_entries[VGPU_RM_CONTROL_TRACE_MAX];
...
void vgpu_ioctl_trace_init(void)
{
	...
	memset(vgpu_rm_control_trace_entries, 0,
	       sizeof(vgpu_rm_control_trace_entries));
...
void vgpu_rm_control_trace_record(__u32 ctrl_cmd, __u32 hobject,
				       ...
				       __s32 gpu_minor, __u32 nvidia_major)
{
	struct vgpu_rm_control_trace_entry *free_entry = NULL;
	struct vgpu_rm_control_trace_entry *entry;
	unsigned long flags;
	size_t i;
	...
	spin_lock_irqsave(&vgpu_ioctl_trace_lock, flags);
	for (i = 0; i < VGPU_RM_CONTROL_TRACE_MAX; i++) {
		entry = &vgpu_rm_control_trace_entries[i];
		if (!entry->used) {
			if (!free_entry)
				free_entry = entry;
			continue;
		}
		if (entry->snapshot.ctrl_cmd == ctrl_cmd)
			goto update;
	}
	...
	entry->used = true;
	entry->snapshot.ctrl_cmd = ctrl_cmd;
	entry->snapshot.count = 0;

update:
	entry->snapshot.count++;
	entry->snapshot.last_hobject = hobject;
	entry->snapshot.last_params_size = params_size;
	entry->snapshot.last_pid = pid;
	entry->snapshot.last_tgid = tgid;
	entry->snapshot.last_gpu_minor = gpu_minor;
	entry->snapshot.last_nvidia_major = nvidia_major;
				       ...
				       size_t max_records)
{
	...
	spin_lock_irqsave(&vgpu_ioctl_trace_lock, flags);
	for (i = 0; i < VGPU_RM_CONTROL_TRACE_MAX && copied < max_records; i++) {
		if (!vgpu_rm_control_trace_entries[i].used)
			continue;
		out[copied] = vgpu_rm_control_trace_entries[i].snapshot;
		copied++;
	}
	spin_unlock_irqrestore(&vgpu_ioctl_trace_lock, flags);
